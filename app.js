	...
	async.auto({
		...
		connect: ["config", "public", "genesisblock", "logger", "build", "network", function (cb, scope) {
			...
			scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
				scope.logger.info("Dlisk started: " + scope.config.address + ":" + scope.config.port);

				if (!err) {
					if (scope.config.ssl.enabled) {
						scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
							scope.logger.info("Dlisk https started: " + scope.config.ssl.options.address + ":" + scope.config.ssl.options.port);
